$(function () {
    var max_file_size = 5242880; // 5 MB
    ...
    document.forms[0].addEventListener('submit', function( evt ) {
        ...
        for(var file = 0 ; file < files.length; file++){
            if(files[file] && files[file].size < max_file_size) { // 5 MB (this size is in bytes)
                total = total + files[file].size;
            } else {
                passed = false;
                //Prevent default and display error
                bootbox.alert('One of the files you uploaded exceeds the limit of 5MB. Please compress the file and try again');
                evt.preventDefault();
            }
        }

        if(passed && total<= post_max_size){
            buttonDisable('upload-button');
        }else{
